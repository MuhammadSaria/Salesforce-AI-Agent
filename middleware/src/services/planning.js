...
export function buildPlan(job, requirement, scope, dependencies) {
  const planCore = {
    ...
    implementationSteps: ['Review the requested behavior and relevant Salesforce configuration', 'Prepare the approved change locally', 'Validate the result against the verified target org without deploying it'],
    expectedOutcome: 'The requested Salesforce behavior is available after separate deployment approval.',
    businessImpact: 'Only the behavior described in the approved Jira requirement will be affected.',
    outOfScope: ['Unrelated Salesforce configuration and data'],
