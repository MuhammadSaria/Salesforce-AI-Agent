...
test('migration 002 upgrades a database that already recorded released 001', async () => {
  ...
  try {
    ...
    assert.deepEqual(migrations.rows.map((row) => row.filename), ['001_phase1_jobs.sql', '002_task6_job_dispatches.sql', '003_task6_dispatch_retry.sql', '004_task10_component_leases.sql', '005_task14_component_lock_sets.sql']);
    ...
    assert.deepEqual(repeated.rows, [
      ...
      { filename: '004_task10_component_leases.sql', count: 1 },
      { filename: '005_task14_component_lock_sets.sql', count: 1 }
    ]);
  } finally {
    await pool.end();
  }
